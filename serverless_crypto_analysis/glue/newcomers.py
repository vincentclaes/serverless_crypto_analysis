...
def dump_newcomers_as_json(newcomers, bucket, dest_key_json):
    ...
    for newcomer in list_of_newcomers:
        coin_id = newcomer['id']
        boto3.client('s3').put_object(Body=json.dumps(newcomer, indent=4, sort_keys=True, default=str), Bucket=bucket,
...
def dump_newcomers_as_parquet(newcomers, bucket, dest_key_parquet):
    if not newcomers.empty:
        ...
        key_with_file_name = os.path.join(bucket, dest_key_parquet, '{}.parquet'.format(uuid))
