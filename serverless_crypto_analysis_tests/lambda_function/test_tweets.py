...
class TestTweets(unittest.TestCase):
    bucket = "bucket"
    rank = 100
    key = "stg/newcomers/rank%3D{}/2019-12-17T21:01:12.717000-qcash.json".format(rank)
    ...
    @mock_s3
    @mock_secretsmanager
    @patch("serverless_crypto_analysis.lambda_function.tweets.get_tweet_id")
    @patch("serverless_crypto_analysis.lambda_function.tweets.get_coinmarketcap_tokens")
    @patch.object(Twython, 'update_status')
    def test_run_several_ids(self, m_status, m_coinmarketcap, m_tweet_id) -> None:
        m_tweet_id.return_value = "tweet-id"
        m_coinmarketcap.return_value = TestTweets.build_coinmarket_response()
        s3_utils.create_bucket(TestTweets.bucket)
        test_file_path = os.path.join(dir_path, "resources", "test_tweets_successfully.json")
        ...
        conn.create_secret(Name=TestTweets.twitter_token, SecretString=json.dumps(twitter_keys))
        conn.create_secret(Name=TestTweets.coinmarketcap_token, SecretString="some-key")
        tweets.lambda_handler(event=TestTweets.build_event(bucket=TestTweets.bucket, key=TestTweets.key), context=None)
